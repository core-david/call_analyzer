...
3. Create `backend/scripts/check_stubs.py` — confirms both stubs return the
   agreed shapes (the contract M2/M3 must keep):
   ...
   ```bash
   cd backend && uv run python scripts/check_stubs.py
   # → shapes OK
   ```
...
- `backend/scripts/check_stubs.py`
